 ...
 *
 * 当 ChoppinessParams.disableCrossings=true 时跳过指标 1，total = bandRatio (0–30)。
 */
...
export interface ChoppinessParams {
  ...
  /**
   * disableCrossings=true 时跳过指标 1，total = bandRatio (0–30)。
   * crossings 字段仍输出（=0），details.crossingCount/Rate 仍计算供诊断，
   * 仅"参与 total 评分"被关闭。默认 false。
   */
  disableCrossings?: boolean;
}
...
export function scoreChoppiness(
  ...
  // disableCrossings=true 时 total 只用 bandRatio；crossings 字段仍计算供诊断
  const effectiveCrossings = params.disableCrossings ? 0 : crossingsScore;

  return {
    total: effectiveCrossings + bandRatioScore,
    crossings: effectiveCrossings,
