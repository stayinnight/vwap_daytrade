...
const realGetTradeProgressMinutes =
  timeGuard.getTradeProgressMinutes.bind(timeGuard);
...
interface RunnerOptions {
  ...
  /**
   * 禁用 choppiness 指标 1（VWAP 穿越频率），仅用指标 2（带内时长比）。
   * 仅影响 scoreChoppiness 的 total 计算（disableCrossings=true）；details 不变。
   * 未指定时回落到 config.choppiness.disableCrossings。
   */
  chopDisableCrossings?: boolean;
 ...
 */
function precomputeAtrByDay(dayBars: DayBar[]): Record<string, number | null> {
...
export async function runBacktest(
  ...
  console.log(`\n[runner] === ${opts.label} ===`);
  console.log(
    `  exitMode=${opts.exitMode} tp=${opts.takeProfitAtrRatio ?? '-'} sl=${
      opts.stopLossAtrRatio ?? '-'
    } resolution=${opts.ambiguousResolution ?? '-'}`
  );
  if (opts.trendAtrShortPeriod !== undefined) {
    console.log(
      `  trendAtrShortPeriod=${opts.trendAtrShortPeriod} trendThreshold=${
        opts.trendThreshold ?? 'default'
      }`
    );
  }
  ...
  if (opts.disableTrendIndicators && opts.disableTrendIndicators.length > 0) {
    for (const n of opts.disableTrendIndicators) {
      if (n === 9) setTrendIndicator9Enabled(false);
        ...
        console.warn(
          `[runner] unknown --disable-trend-ind value: ${n}, ignored`
        );
    }
  }
  ...
  if (indexEnabled) {
    ...
    console.log(`[runner] 加载指数 ${indexSymbol} ${indexBars!.length} 根`);
  }
  ...
  const trendBaselineBySymbol: Record<
    string,
    Record<string, TrendBaseline | null>
  > = {};
  const firstIntradayBarIndexBySymbol: Record<
    string,
    Record<string, number>
  > = {};
  for (const { symbol, bars } of allData) {
    trendBaselineBySymbol[symbol] = precomputeTrendBaselinesForSymbol(
      bars,
      opts.trendAtrShortPeriod
    );
  ...
  const pendingEntry: Record<
    string,
    { side: OrderSide; chopScore: BacktestTrade['entryChopScore'] }
  > = {};
  ...
  for (const ts of timestamps) {
    ...
    for (const { symbol, index } of tickMap[ts]) {
      ...
      if (
        dayScoreMap[symbol] === undefined &&
        minutesSinceOpen >= OPENING_WINDOW_MINUTES
      ) {
        ...
        if (opts.exitMode === 'fixed') {
          ...
          if (pos.stopDistance !== null) {
            ...
            if (pos.side === OrderSide.Buy) {
              pos.stopPrice = Math.max(pos.stopPrice, c - pos.stopDistance);
            } else {
              pos.stopPrice = Math.min(pos.stopPrice, c + pos.stopDistance);
            }
          }
        }
      }
      ...
      if (!positions[symbol] && pendingEntry[symbol]) {
        ...
        if (a && a > 0) {
          ...
          const newPos: Position = {
            ...
            entryDayScore:
              scoreNow && typeof scoreNow === 'object' ? scoreNow.total : null,
            entryDayScoreDetail:
      ...
      if (!positions[symbol] && inTradeWindow && !isForceCloseWindow) {
        ...
        const windowBars =
          market.getBarAt(symbol, 0) === undefined
            ? []
            : (() => {
        ...
        if (a && a > 0 && Number.isFinite(vwap)) {
          ...
          // 日内震荡评分（仅在启用时算，与 onBar 完全对齐）
          // CLI flag --chop-disable-ind1 优先；未传时 fallback 读 config.choppiness.disableCrossings
          const chopScore = config.filters.enableChoppiness
            ? scoreChoppiness(
                ...
                a,
                {
                  ...
                  disableCrossings:
                    opts.chopDisableCrossings === true ||
                    config.choppiness.disableCrossings === true,
                }
              )
            : null;
  ...
  console.log(
    `[runner] 完成 ${opts.label}  交易数=${trades.length}  -> ${path.relative(
      process.cwd(),
      outPath
    )}`
  );
...
async function main() {
  ...
  if (!label || !exitMode) {
    console.error(
      'Usage: runner.ts <label> <trailing|fixed> [tp] [sl] [SLFirst|TPFirst]\n' +
        ...
        '  [--filter-choppiness=on|off] [--chop-window=N] [--chop-threshold=N] [--chop-disable-ind1]\n' +
  ...
  if (disableTrendIndRaw) {
    const tokens = String(disableTrendIndRaw)
      .split(',')
      .map((s) => s.trim())
      .filter((s) => s.length > 0);
    ...
    const invalid = tokens.filter(
      (_t, i) =>
        !Number.isInteger(parsed[i]) || ![9, 10, 11].includes(parsed[i])
    );
    if (invalid.length > 0 || parsed.length === 0) {
      console.error(
        `[runner] invalid --disable-trend-ind=${disableTrendIndRaw}, invalid tokens: [${invalid.join(
          ','
        )}]; expected comma-separated list of 9/10/11`
      );
      process.exit(1);
    }
  ...
  if (ind11ModeFlag) {
    if (['forward', 'reverse', 'range', 'off'].includes(ind11ModeFlag)) {
      ...
      console.error(
        `[runner] invalid --ind11-mode=${ind11ModeFlag}, expected forward|reverse|range|off`
      );
      process.exit(1);
    }
  }
  ...
  if (
    disableTrendIndicators?.includes(11) &&
    ind11Mode &&
    ind11Mode !== 'off'
  ) {
    console.error(
  ...
  const opts: RunnerOptions = {
    ...
    ambiguousResolution: resolution === 'TPFirst' ? 'TPFirst' : 'SLFirst',
    stopAtrRatio:
      flags['stop-atr'] !== undefined ? Number(flags['stop-atr']) : undefined,
    filters:
    ...
    chopScoreThreshold:
      flags['chop-threshold'] !== undefined
        ...
        : undefined,
    chopDisableCrossings: flags['chop-disable-ind1'] === true,
  };

  // 一致性检查：--chop-window / --chop-threshold / --chop-disable-ind1 必须配合 --filter-choppiness=on
  if (
    (opts.chopWindowBars !== undefined ||
      opts.chopScoreThreshold !== undefined ||
      opts.chopDisableCrossings === true) &&
    opts.filters?.enableChoppiness !== true
  ) {
    console.error(
      '[runner] --chop-window / --chop-threshold / --chop-disable-ind1 需要配合 --filter-choppiness=on 才会生效\n' +
