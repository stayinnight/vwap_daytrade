...
class VWAPStrategy {
  ...
  private logEntryPriceTriggerOnce(params: {
    ...
    logger.info(
      `\n🚀【入场触发-价格】${params.symbol} 方向=${params.dirText} 时段=${
        params.phaseText
      } K线=${params.barTimeStr}  结论=${
        ...
        `  动量：个股VWAP斜率=${this.fmtMaybe(params.slopeBps, 2)}bps ${
          params.momentumRule
        } 结果=${params.momentumResult}\n` +
  ...
  canOpen(
    ...
    const { low: lastOneMinuteslow, top: lastOneMinutesHigh } = calcTopAndLow([
      lastOneMinutesBar,
    ]);
    const { low: lastTwoMinuteslow, top: lastTwoMinutesHigh } = calcTopAndLow([
      lastTwoMinutesBar,
    ]);
    ...
    const slopeOkLong = !isTrendEnabled
      ? true
      : indexSlope === null
      ? trendCfg.whenSlopeUnavailable !== 'block'
      : indexSlope > epsilon;
    const slopeOkShort = !isTrendEnabled
      ? true
      : indexSlope === null
      ? trendCfg.whenSlopeUnavailable !== 'block'
      : indexSlope < -epsilon;
    ...
    const slopeBps =
      symbolSlope !== null && vwap > 0 ? (symbolSlope / vwap) * 10000 : null;
    const momentumOk =
      !isMomentumEnabled ||
    ...
    if (longPriceTrigger) {
      ...
      this.logEntryPriceTriggerOnce({
        ...
        priceLine:
          `上轨=${this.fmtNumber(upperBand)} (VWAP=${this.fmtNumber(
            vwap
          )} ATR=${this.fmtNumber(atr)} k=${k}) ` +
          `last2Low=${this.fmtNumber(
            lastTwoMinuteslow
          )} -> last1Low=${this.fmtNumber(lastOneMinuteslow)}`,
        rsi,
        ...
        key: `L:${barTimeStr}:${this.fmtNumber(upperBand)}:${this.fmtNumber(
          lastOneMinuteslow
        )}`,
      });
      ...
      this.logEntryPriceTriggerOnce({
        ...
        priceLine:
          `下轨=${this.fmtNumber(lowerBand)} (VWAP=${this.fmtNumber(
            vwap
          )} ATR=${this.fmtNumber(atr)} k=${k}) ` +
          `last2High=${this.fmtNumber(
            lastTwoMinutesHigh
          )} -> last1High=${this.fmtNumber(lastOneMinutesHigh)}`,
        rsi,
        rsiRule: shouldCheckIndicators
        ...
        key: `S:${barTimeStr}:${this.fmtNumber(lowerBand)}:${this.fmtNumber(
          lastOneMinutesHigh
        )}`,
      });
    }
    ...
    const chopScore = filters.enableChoppiness
      ? scoreChoppiness(
          ...
          atr,
          {
            ...
            disableCrossings: this.config.choppiness.disableCrossings,
          }
        )
      : null;
  ...
  async managePosition(
    ...
    const logAndClose = async (reason: string) => {
      const pnl =
        state.entryPrice !== null ? dir * (currPrice - state.entryPrice) : null;
      const pnlPct =
      ...
      logger.info(
        `\n🛑【${reason}】${symbol} 持仓=${posText} 时间=${nowStr}\n` +
          `  价格：现价=${this.fmtNumber(currPrice, 4)} 止损价=${this.fmtMaybe(
            state.stopPrice,
            4
          )} 止盈价=${this.fmtMaybe(state.tpPrice, 4)}\n` +
          `  仓位：数量=${state.qty} 入场价=${this.fmtMaybe(
            state.entryPrice,
            4
          )} VWAP=${this.fmtNumber(vwap, 4)} ATR=${this.fmtNumber(atr, 4)}\n` +
          `  盈亏：每股=${
            pnl === null ? '未知' : this.fmtNumber(pnl, 4)
          } 比例=${
            pnlPct === null ? '未知' : (pnlPct * 100).toFixed(2) + '%'
          }\n`
      );
      await placeOrder({
        symbol,
    ...
    if (exitMode === 'fixed') {
      ...
      const slHit =
        state.stopPrice !== null && dir * (currPrice - state.stopPrice) <= 0;
      const tpHit =
        state.tpPrice !== null && dir * (currPrice - state.tpPrice) >= 0;
